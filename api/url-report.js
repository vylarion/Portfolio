// Vercel Serverless Function: Get URL report by URL ID
...
export default async function handler(req, res) {
    ...
    try {
        ...
        // Compute VT URL ID: base64(url) without trailing '='
        const urlId = Buffer.from(url).toString("base64").replace(/=+$/, "");

        const result = await vtGet("/api/v3/urls/" + encodeURIComponent(urlId), apiKey);
        ...
        console.error("URL report error:", err);
